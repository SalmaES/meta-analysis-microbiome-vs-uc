##Preprocessing data script for Wang et al., 2020
#######################################################################################################
...
path <- "~/16S_mouse_data/WANG/PRJNA596333"
...
pdf(paste0(fnRs,"qualityprofiles_Rs_WANG.pdf"), width=10, height=10)
plotQualityProfile(fnRs[1:5])
dev.off()
pdf(paste0(fnFs,"qualityprofiles_Fs_WANG.pdf"), width=10, height=10)
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft=15, truncLen = c(200, 200), maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
save(out, file="filterandtrim_WANG.Rda") #Saving the output 


#Estimate the error model for DADA2 algorithm using reverse reads
errR <- learnErrors(filtRs, multithread = TRUE)
save(errR, file="errorsR_WANG.Rda") #Saving the output 
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_WANG.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_WANG.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()
#Plot error rates for all possible base transitions
pdf("errorplotsR_WANG.pdf", width=10, height=10)
...
save(derepFs, file="dereplicationFs_WANG.Rda") #Saving the output 
derepRs <- derepFastq(filtRs, verbose = TRUE)
save(derepRs, file="dereplicationRs_WANG.Rda") #Saving the output 
...
save(dadaRs, file="inferenceRs_WANG.Rda") #Saving the output 
dadaFs <- dada(derepFs, err = errF, multithread = TRUE)
save(dadaFs, file="inferenceFs_WANG.Rda") #Saving the output


#Merge denoised reads
mergers <- mergePairs(dadaFs, derepFs, dadaRs, derepRs, verbose = TRUE)
head(mergers[[1]])
save(mergers, file="mergeddenoisedreads_WANG.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_WANG.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_WANG.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_WANG.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_WANG.txt", sep="\t")   
...
write(asv_fasta, "ASVs_WANG.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_WANG.tsv", sep="\t", quote=F, col.names=NA)
