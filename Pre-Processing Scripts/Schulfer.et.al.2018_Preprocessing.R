##Preprocessing data script for Schulfer et al., 2018
#######################################################################################################
...
path <- "~/16S_mouse_data/SCHULFER/ERP104982"
...
pdf(paste0(fnFs,"qualityprofiles_SCHULFER.pdf"), width=10, height=10)
...
out <- filterAndTrim(fnFs, filtFs, trimLeft=15, truncLen = 135, maxN=0, maxEE=2, truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
save(out, file="filterandtrim_SCHULFER.Rda") #Saving the output 


#Estimate the error model for DADA2 algorithm using reverse reads
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_SCHULFER.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_SCHULFER.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()

#Dereplicate FASTQ files to speed up computation
derepFs <- derepFastq(filtFs, verbose = TRUE)
save(derepFs, file="dereplicationFs_SCHULFER.Rda") #Saving the output 
...
save(dadaFs, file="inferenceFs_SCHULFER.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_SUNSCHULFER.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_SCHULFER.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_SCHULFER.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_SCHULFER.txt", sep="\t")   
...
write(asv_fasta, "ASVs_SUN_D.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_SCHULFER.tsv", sep="\t", quote=F, col.names=NA)
