##Analysis Script for Spalinger et al., 2019
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_SPALINGER.rds") #feature counts table
taxa <- read_csv("taxtab_SPALINGER.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_SPALINGER.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_SPALINGER.rds") #phylogenetic tree
...
ps
saveRDS(ps, file = "phyloseq_object_raw_SPALINGER.rds")
...
taxa_names(ps2)[1:2]
saveRDS(ps2, file = "phyloseq_raw_no_outliers_SPALINGER.rds")

#Rarefy the data
set.seed(19920921)
ps.rarefied <- rarefy_even_depth(ps2, rngseed=FALSE, replace=T)
saveRDS(ps.rarefied, file = "phyloseq_rarefied_SPALINGER.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_SPALINGER.pdf")
...
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_SPALINGER.rds")
...
wtp2=pairwise.wilcox.test(rich.pruned$Observed, sample_data(ps.rarefied.pruned)$Length_of_Treatment)

write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(condition).SPALINGER.csv")
write.csv(wtp2[["p.value"]], "Wilcoxon.test.rarefied.pruned(LengthOfTreatment).SPALINGER.csv")

#Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_SPALINGER.pdf")
...
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(LengthOfTreatment)_SPALINGER.pdf")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_SPALINGER.rds")

ps.rarefied.pruned1 <- readRDS("phyloseq_rarefied_pruned1_SPALINGER.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.SPALINGER.csv")

beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Length_of_Treatment", distance = "bray") 
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned(LengthOfTreatment).SPALINGER.csv")
...
write.csv(deseq2_table, "DESeq2_table(Length_of_Treatment0vs79)_SPALINGER.csv")
...
df1 <- subset (df, ef_logFC >= 22 | ef_logFC <= -22)

write_csv(df1, file = "DESeq2_marker_table_22log2FC_SPALINGER.csv")
...
annotation_colours <- list(
  ...
  'Length_of_Treatment' = c("0_days" =colors_blue, "8_days" = colors_lilac,
                            "12_days" =colors_pink, "16_days" =colors_green, "18_days" = colors_yellow,
                            "26_days"=colors_orange, "79_days"=colors_red)
)
colAnnotation <- HeatmapAnnotation(
  ...
  annotation_legend_param =list(
    Length_of_Treatment = list(
      ...
      at = c("0_days", "8_days", "12_days", "16_days", "18_days", "26_days", "79_days"),
      labels = c("0 days", "8 days", "12 days", "16 days", "18 days", "26 days", "79 days")
    )))
...
pdf(file="DESeq2_Heatmap_SPALINGER(Length_of_Treatment0vs79+log10).pdf", height=15)
