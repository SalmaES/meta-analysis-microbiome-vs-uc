##Preprocessing data script for Khan et al., 2020
#######################################################################################################
...
pdf(paste0(fnRs,"qualityprofiles_Rs_KHAN.pdf"), width=10, height=10)
plotQualityProfile(fnRs[1:2])
dev.off()
pdf(paste0(fnFs,"qualityprofiles_Fs_KHAN.pdf"), width=10, height=10)
...
save(out, file="filterandtrim_KHAN.Rda") #Saving the output 
...
save(errR, file="errorsR_KHAN.Rda") #Saving the output 
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_LEONARDI.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_KHAN.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()
#Plot error rates for all possible base transitions
pdf("errorplotsR_KHAN.pdf", width=10, height=10)
...
save(derepFs, file="dereplicationFs_KHAN.Rda") #Saving the output 
derepRs <- derepFastq(filtRs, verbose = TRUE)
save(derepRs, file="dereplicationRs_KHAN.Rda") #Saving the output 
...
save(dadaRs, file="inferenceRs_KHAN.Rda") #Saving the output 
dadaFs <- dada(derepFs, err = errF, multithread = TRUE)
save(dadaFs, file="inferenceFs_KHAN.Rda") #Saving the output


#Merge denoised reads
mergers <- mergePairs(dadaFs, derepFs, dadaRs, derepRs, verbose = TRUE)
head(mergers[[1]])
save(mergers, file="mergeddenoisedreads_KHAN.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_KHAN.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_KHAN.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_KHAN.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_KHAN.txt", sep="\t")   
...
write(asv_fasta, "ASVs_KHAN.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_KHAN.tsv", sep="\t", quote=F, col.names=NA)