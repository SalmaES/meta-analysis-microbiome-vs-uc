##Analysis Script for Roy et al., 2017
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_F_ROY_NO.rds") #feature counts table
taxa <- read_csv("taxtab_F_ROY_NO.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_ROY.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_F_ROY_NO.rds") #phylogenetic tree
...
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers_ROY.rds")
...
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_ROY.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_ROY.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_ROY.rds")
...
wtp3=pairwise.wilcox.test(rich.pruned$Observed, sample_data(ps.rarefied.pruned)$Treatment)
wtp4=pairwise.wilcox.test(rich.pruned$Observed, sample_data(ps.rarefied.pruned)$IntensityOfDisease)

write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Condition).ROY.csv")
write.csv(wtp2[["p.value"]], "Wilcoxon.test.rarefied.pruned(Treatment).ROY.csv")
write.csv(wtp4[["p.value"]], "Wilcoxon.test.rarefied.pruned(IntensityOfDisease).ROY.csv")

#Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_ROY.pdf")
plot_ordination(ps.rarefied.pruned, ordination.rarefied.pruned, color = "Condition", 
                title = "ASVs_Rarefied+Pruned(Bray)") + theme(aspect.ratio=1)
dev.off()

ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(treatment)_ROY.pdf")
plot_ordination(ps.rarefied.pruned, ordination.rarefied.pruned, color = "Treatment", 
                title = "ASVs_Rarefied+Pruned(Bray)") + theme(aspect.ratio=1)
dev.off()

ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(IntensityOfDisease)_ROY.pdf")
plot_ordination(ps.rarefied.pruned, ordination.rarefied.pruned, color = "Intensity_of_Disease", 
                title = "ASVs_Rarefied+Pruned(Bray)") + theme(aspect.ratio=1)
dev.off()
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_ROY.rds")
ps.rarefied.pruned1 <- readRDS("phyloseq_rarefied_pruned1_ROY.rds")

#PERMANOVA testing (p value for PCoA plots)
beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Condition", distance = "bray") 
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned(Condition).ROY.csv")

beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Treatment", distance = "bray") 
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned(treatment).ROY.csv")
   
beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Intensity_of_Disease", distance = "bray") 
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned(IntensityOfDisease).ROY.csv")

#Differential abundance testing with DESeq2
deseq2 <- microbiomeMarker::run_deseq2(ps.rarefied.pruned1,
                                       group = c("Intensity_of_Disease"),
                                       contrast = c("Mild", "Severe"),                                       taxa_rank = "none",
                                       sfType = "poscounts",
                                       pvalue_cutoff=0.01, 
                                       transform = "identity", 
                                       p_adjust = "fdr")
...
write.csv(deseq2_table, "DESeq2_table(Intensity_of_Disease)_ROY.csv")
...
deseq2.2 <- transform_abundances(deseq2, transform = "identity")
df <- marker_table(deseq2.2)
df1 <- subset (df, ef_logFC >= 5 | ef_logFC <= -5)
...
annotationData <- data.frame(sampledata$Condition, sampledata$Treatment, sampledata$Intensity_of_Disease, sampledata$Sample_Accession)
rownames(annotationData) <- annotationData[,4]
...
annotationData <- annotationData[annotationData$sampledata.Sample_Accession %in% abundance_colnames,1:3]
colnames(annotationData) <- c('Condition', 'Treatment', 'Intensity_of_Disease')
annotation_colours <- list(
  'Condition' = c("Colitogenic" = "#FFC300", "Healthy" = "#1872B0","Colitis" = "#FA7F4A"),
  'Treatment' = c("SPF-1" =colors_blue, "cDysN6" = colors_lilac, "SPF-2" =colors_pink, 
                  "SPF-4" =colors_green, "SPF-5" = colors_yellow, "cSPF-2"=colors_orange, 
                  "cSPF-2+DSS"=colors_teal, "SPF-6"=colors_red, "SPF-3"=colors_purple, 
                  "cDysN6+SPF-2"=colors_grey, "DysN6+gavage"=colors_lime,"DysN6"=colors_brown),
  'Intensity_of_Disease' = c("Mild" = colors_lime, "Severe"= colors_red)
  )
colAnnotation <- HeatmapAnnotation(
  ...
  annotation_name_gp = gpar(
    ...
    annotation_legend_param =list(
      Intensity_of_Disease = list(
        title = "Intensity of Disease",
        title_position = "topleft",
        labels_gp = gpar(fontsize = 9)

  )))

col_fun = colorRamp2(c(0,1500), c("white", "#C70039"))
...
hmap <- Heatmap(
  ...
  column_order = order(annotationData$Condition, annotationData$Treatment, annotationData$Intensity_of_Disease),
...
pdf(file="DESeq2_Heatmap_ROY(Intensity_of_Diseasett).pdf", height=10)
