##Analysis Script for Song et al., 2019
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_150_SONG.rds") #feature counts table
taxa <- read_csv("taxtab_150_SONG.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_SONG.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_150_SONG.rds") #phylogenetic tree
...
ps
saveRDS(ps, file = "phyloseq_object_raw_SONG.rds")
...
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers_SONG.rds")
...
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_SONG.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_SONG.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_SONG.rds")
...
write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Condition).SONG.csv")


#Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_SONG.pdf")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_SONG.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.SONG.csv")
...
write.csv(deseq2_table, "DESeq2_table(Condition)_SONG.csv")
...
write_csv(df1, "DESeq2_marker_table_12log10FC_SONG.csv")
...
pdf(file="DESeq2_Heatmap_SONG(Condition+log10).pdf")
...
annotation_colours <- list(
  'Condition' = c("Colitogenic" = "#FFC300", "Healthy" = "#1872B0"),
  'Treatment' = c("Rich_Diet" =colors_pink, "Minimal_Diet" =colors_green)
)
colAnnotation <- HeatmapAnnotation(
  df = annotationData,
  which = 'col',
  ...
  annotation_legend_param =list(
    Treatment = list(
      ...
      title_position = "topleft",
      labels_gp = gpar(fontsize = 9),
      at = c("Rich_Diet", "Minimal_Diet"),
      labels = c("Rich Diet", "Minimal Diet")
    )))
...
pdf(file="DESeq2_Heatmap_SONG(Condition+treatmentbar+log10).pdf")
draw(hmap, merge_legend = TRUE, legend_grouping = "original")
dev.off()
