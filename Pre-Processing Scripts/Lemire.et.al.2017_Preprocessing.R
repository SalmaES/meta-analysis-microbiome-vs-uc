##Preprocessing data script for Britton et al., 2017
#######################################################################################################
...
pdf(paste0(fnRs,"qualityprofiles_Rs_LEMIRE.pdf"), width=10, height=10)
plotQualityProfile(fnRs[1:2])
dev.off()
pdf(paste0(fnFs,"qualityprofiles_Fs_LEMIRE.pdf"), width=10, height=10)
...
save(out, file="filterandtrim_LEMIRE.Rda") #Saving the output 
...
save(errR, file="errorsR_LEMIRE.Rda") #Saving the output 
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_LEMIRE.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_LEMIRE.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()
#Plot error rates for all possible base transitions
pdf("errorplotsR_LEMIRE.pdf", width=10, height=10)
...
save(derepFs, file="dereplicationFs_LEMIRE.Rda") #Saving the output 
derepRs <- derepFastq(filtRs, verbose = TRUE)
save(derepRs, file="dereplicationRs_LEMIRE.Rda") #Saving the output 
...
save(dadaRs, file="inferenceRs_LEMIRE.Rda") #Saving the output 
dadaFs <- dada(derepFs, err = errF, multithread = TRUE)
save(dadaFs, file="inferenceFs_LEMIRE.Rda") #Saving the output


#Merge denoised reads
mergers <- mergePairs(dadaFs, derepFs, dadaRs, derepRs, verbose = TRUE)
head(mergers[[1]])
save(mergers, file="mergeddenoisedreads_LEMIRE.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_LEMIRE.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_LEMIRE.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_LEMIRE.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_LEMIRE.txt", sep="\t")   
...
write(asv_fasta, "ASVs_LEMIRE.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_LEMIRE.tsv", sep="\t", quote=F, col.names=NA)