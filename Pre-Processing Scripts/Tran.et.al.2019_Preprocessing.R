##Preprocessing data script for Tran et al., 2019
#######################################################################################################
...
path <- "~/16S_mouse_data/TRAN/PRJEB35012"
...
pdf(paste0(fnFs,"qualityprofiles_Fs_TRAN.pdf"), width=10, height=10)
...
out <- filterAndTrim(fnFs, filtFs, trimLeft=15, truncLen = 250, maxN=0, maxEE=2, truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
save(out, file="filterandtrim_TRAN.Rda") #Saving the output 


#Estimate the error model for DADA2 algorithm using reverse reads
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_TRAN.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_TRAN.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()

#Dereplicate FASTQ files to speed up computation
derepFs <- derepFastq(filtFs, verbose = TRUE)
save(derepFs, file="dereplicationFs_TRAN.Rda") #Saving the output 
...
save(dadaFs, file="inferenceFs_TRAN.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_TRAN.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_TRAN.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_TRAN.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_TRAN.txt", sep="\t")   
...
write(asv_fasta, "ASVs_TRAN.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_TRAN.tsv", sep="\t", quote=F, col.names=NA)
