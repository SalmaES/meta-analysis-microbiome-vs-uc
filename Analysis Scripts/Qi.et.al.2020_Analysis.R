...
OTU <- readRDS("seqtabnoc_F_QI.rds") #feature counts table
taxa <- read_csv("taxtab_F_QI.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_QI.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_F_QI.rds") #phylogenetic tree
...
saveRDS(ps, file = "phyloseq_object_raw_QI.rds")
...
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers_QI.rds")
...
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_QI.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_QI.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_QI.rds")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_QI.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.QI.csv")

#Differential abundance testing using DESeq2
ps.rarefied.pruned1 <- readRDS("phyloseq_rarefied_pruned1_QI.rds")
sampledata <- sample_data(ps.rarefied.pruned1)
deseq2 <- microbiomeMarker::run_deseq2(ps.rarefied.pruned1,
                                       group = "Condition",
                                       contrast = c("Healthy", "Colitis"),
                                       taxa_rank = "none",
                                       sfType = "poscounts",
                                       pvalue_cutoff=0.01, 
                                       transform = "identity", 
...
write.csv(deseq2_table, "DESeq2_table(Condition)_QI.csv")
...
df <- marker_table(deseq2.2) # 91 obs of 5 variables including feature. Rownames are marker
df1 <- subset (df, ef_logFC >= 12 | ef_logFC <= -12)

write_csv(df1, "DESeq2_marker_table_12log10FC_QI.csv")

abd <- as(otu_table(deseq2.2), "matrix") # abd is a 38 column matrix with 1035 rows , columns have sample_Accession as column name
marker_abd <- abd[match(df1$feature, row.names(abd)), ] %>%
  as.data.frame() # 67 obs of 38 variables, 38 columns, columns have sample_Accession as column name
...
col_fun = colorRamp2(c(0,4.5), c("white", "#C70039"))
...
pdf(file="DESeq2_Heatmap(Condition+log10)_QI.pdf", height=12)
