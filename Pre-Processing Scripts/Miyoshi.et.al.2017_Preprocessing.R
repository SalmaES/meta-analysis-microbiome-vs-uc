##Preprocessing data script for Miyoshi et al., 2017
#######################################################################################################
...
pdf(paste0(fnRs,"qualityprofiles_Rs_MIYOSHI.pdf"), width=10, height=10)
plotQualityProfile(fnRs[1:2])
dev.off()
pdf(paste0(fnFs,"qualityprofiles_Fs_MIYOSHI.pdf"), width=10, height=10)
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen = c(250, 225), maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
save(out, file="filterandtrim_MIYOSHI.Rda") #Saving the output 
...
save(errR, file="errorsR_MIYOSHI.Rda") #Saving the output 
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_MIYOSHI.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_MIYOSHI.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()
#Plot error rates for all possible base transitions
pdf("errorplotsR_MIYOSHI.pdf", width=10, height=10)
...
save(derepFs, file="dereplicationFs_MIYOSHI.Rda") #Saving the output 
derepRs <- derepFastq(filtRs, verbose = TRUE)
save(derepRs, file="dereplicationRs_MIYOSHI.Rda") #Saving the output 
...
save(dadaRs, file="inferenceRs_MIYOSHI.Rda") #Saving the output 
dadaFs <- dada(derepFs, err = errF, multithread = TRUE)
save(dadaFs, file="inferenceFs_MIYOSHI.Rda") #Saving the output


#Merge denoised reads
mergers <- mergePairs(dadaFs, derepFs, dadaRs, derepRs, verbose = TRUE)
head(mergers[[1]])
save(mergers, file="mergeddenoisedreads_MIYOSHI.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_MIYOSHI.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_MIYOSHI.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_MIYOSHI.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_MIYOSHI.txt", sep="\t")   
...
write(asv_fasta, "ASVs_MIYOSHI.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_MIYOSHI.tsv", sep="\t", quote=F, col.names=NA)