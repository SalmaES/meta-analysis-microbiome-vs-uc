##Analysis Script for Kazakevych et al., 2020
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
colors_pink <- "#FCCDE5"
colors_green <- "#8DD3C7"
colors_all <- c(colors_teal, colors_red, colors_blue, colors_purple, colors_green, colors_yellow, colors_brown, colors_grey)

########################### LOADING DATASETS ########################### 

OTU <- readRDS("seqtabnoc_KAZAKEVYCH.rds")   #feature counts table
taxa <- read_csv("taxtab_KAZAKEVYCH.csv")  #taxonomy table
sampledata <- read_csv("adjusted_sampledata_KAZAKEVYCH.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_KAZAKEVYCH.rds") #phylogenetic tree
...
#Renaming OTU table rownames
rownames(OTU) <- rownames
...
saveRDS(ps, file = "phyloseq_object_raw.rds")
...
ps1 <- ps %>%
  ...
  speedyseq::rename_with_tax_table(stringr::str_to_title)

#Adding the lowest taxonomy for an ASV
ps2 <- add_besthit(ps1, sep=":")
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers.rds")
...
#Merging ASVs at Phylum level for a cleaner barplot
ps.phylum <- tax_glom(ps2, taxrank="Phylum", NArm=FALSE)
ps.phylum.rarefied <- tax_glom(ps.rarefied, taxrank = "Phylum", NArm=FALSE)
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_KAZAKEVYCH.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned.rds")
...
wtp2=pairwise.wilcox.test(rich.pruned$Observed, sample_data(ps.rarefied.pruned)$Treatment)
wtp3=pairwise.wilcox.test(rich.pruned$Observed, sample_data(ps.rarefied.pruned)$Genotype)
wtp4=pairwise.wilcox.test(rich.pruned$Observed, sample_data(ps.rarefied.pruned)$Intensity_of_Disease)

write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Condition).KAZAKEVYCH.csv")
write.csv(wtp2[["p.value"]], "Wilcoxon.test.rarefied.pruned(Treatment).KAZAKEVYCH.csv")
write.csv(wtp3[["p.value"]], "Wilcoxon.test.rarefied.pruned(Genotype).KAZAKEVYCH.csv")
write.csv(wtp4[["p.value"]], "Wilcoxon.test.rarefied.pruned(Intensity_of_Disease).KAZAKEVYCH.csv")

##Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_KAZAKEVYCH.pdf")
plot_ordination(ps.rarefied.pruned, ordination.rarefied.pruned, color = "Condition", 
                title = "ASVs_Rarefied+Pruned") + theme(aspect.ratio=1)
dev.off()



ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(Treatment)_KAZAKEVYCH.pdf")
...
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1.rds")
ps.rarefied.pruned1 <- readRDS("phyloseq_rarefied_pruned1.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.KAZAKEVYCH.csv")

beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Treatment", distance = "bray") 

write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied(Treatment).pruned.KAZAKEVYCH.csv")
...
deseq2 <- microbiomeMarker::run_deseq2(ps.rarefied.pruned1,
                                       group = "Treatment",
                                       contrast = c("No_cohousing", "Cohousing"),
                                       taxa_rank = "none",
                                       sfType = "poscounts",
                                       pvalue_cutoff=0.01, 
                                       transform = "identity", 
...
write.csv(deseq2_table, "DESeq2_table(Treatment.cohousevsnocohouse)_KAZAKEVYCH.csv")
...
marker_abd <- abd[match(df$feature, row.names(abd)), ] %>%
  as.data.frame()
...
annotationData <- data.frame(sampledata$Condition, sampledata$Treatment, sampledata$Sample_Accession)
...
colnames(annotationData) <- c('Condition', 'Treatment')
annotation_colours <- list(
  'Condition' = c("Colitogenic" = "#FFC300", "Healthy" = "#1872B0"),
  'Treatment' = c("No_treatment" =colors_blue, 
                  "No_cohousing" =colors_pink, "Cohousing" =colors_green)
)
colAnnotation <- HeatmapAnnotation(
  ...
  annotation_legend_param =list(
    Treatment = list(
      ...
      at = c("No_treatment", "No_cohousing", "Cohousing"),
      labels = c("No treatment", "No cohousing", "Cohousing")
    )))


max(marker_abd)
col_fun = colorRamp2(c(0,4), c("white", "#C70039"))
...
hmap <- Heatmap(
  ...
  column_order = order(annotationData$Condition, annotationData$Treatment),
...
pdf(file="DESeq2_Heatmap_KAZAKEVYCH(Treatment.cohousevsnocohouse+log10).pdf", height=10)
