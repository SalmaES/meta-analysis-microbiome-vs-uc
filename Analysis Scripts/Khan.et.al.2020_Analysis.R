##Analysis Script for Khan et al., 2020
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_F_KHAN.rds") #feature counts table
taxa <- read_csv("taxtab_F_KHAN.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_KHAN.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_F_KHAN.rds") #phylogenetic tree
...
ps
saveRDS(ps, file = "phyloseq_object_raw_KHAN.rds")
...
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers_KHAN.rds")
...
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_KHAN.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_KHAN.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_KHAN.rds")
...
write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Condition).KHAN.csv")
write.csv(wtp2[["p.value"]], "Wilcoxon.test.rarefied.pruned(Treatment).KHAN.csv")
write.csv(wtp3[["p.value"]], "Wilcoxon.test.rarefied.pruned(Intensity_of_Disease).KHAN.csv")

##Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_KHAN.pdf")
...
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(treatment)_KHAN.pdf")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_KHAN.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.KHAN.csv")

beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Treatment", distance = "bray") 
write.csv(beta.rarified.unifrac, "PERMANOVA.test.unifrac.rarefied.pruned(treatment).KHAN.csv")
...
write.csv(deseq2_table, "DESeq2_table(Condition)_KHAN.csv")
...
annotation_colours <- list(
  ...
  'Treatment' = c("Control_Diet" =colors_blue, 
                  "Fructose_Diet" =colors_pink, "Glucose_Diet" =colors_green,
                  "Sucrose_Diet" =colors_yellow)
)
colAnnotation <- HeatmapAnnotation(
  ...
  annotation_legend_param =list(
    Treatment = list(
      ...
      at = c("Control_Diet", "Fructose_Diet", "Glucose_Diet", "Sucrose_Diet"),
      labels = c("Control Diet", "Fructose Diet", "Glucose Diet", "Sucrose Diet")
    )))
...
pdf(file="DESeq2_Heatmap_KHAN(Condition).pdf")
