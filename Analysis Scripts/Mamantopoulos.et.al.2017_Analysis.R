##Analysis Script for Mamantopoulos et al., 2017
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_F_MAMANTOPOULOS.rds") #feature counts table
taxa <- read_csv("taxtab_F_MAMANTOPOULOS.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_MAMANTOPOULOS.csv") #metadata #had to change the genotype in sampledata from -/- to ko, -/+ to d and +/+ to w for DESeq to work
tree <- readRDS("Phylogenetic_Tree_F_MAMANTOPOULOS.rds") #phylogenetic tree
...
ps
saveRDS(ps, file = "phyloseq_object_raw_MAMANTOPOULOS.rds")
...
taxa_names(ps2)[1:2]
saveRDS(ps2, file = "phyloseq_raw_no_outliers_MAMANTOPOULOS.rds")
...
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_MAMANTOPOULOS.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_MAMANTOPOULOS.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_MAMANTOPOULOS.rds")

########################### DATA ANALYSIS ########################### 
#Pairwise testing for rarefied and pruned data using Wilcoxon test
rich.pruned = estimate_richness(ps.rarefied.pruned)
wtp1=pairwise.wilcox.test(rich.pruned$Observed, sample_data(ps.rarefied.pruned)$Genotype)

write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Genotype).MAMANTOPOULOS.csv")

##Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(genotype)_MAMANTOPOULOS.pdf")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_MAMANTOPOULOS.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned(genotype).MAMANTOPOULOS.csv")



#Differential abundance testing using DESeq2
ps.rarefied.pruned1 <- readRDS("phyloseq_rarefied_pruned1_MAMANTOPOULOS.rds")
sampledata <- sample_data(ps.rarefied.pruned1)
deseq2 <- microbiomeMarker::run_deseq2(ps.rarefied.pruned1,
                                       group = "Genotype",
                                       contrast = c("Nlrp6WT", "Nlrp6KO"),
                                       taxa_rank = "none",
                                       sfType = "poscounts",
                                       pvalue_cutoff=0.01, 
                                       transform = "identity", 
...
write.csv(deseq2_table, "DESeq2_table(Genotype)_MAMANTOPOULOS.csv")
...
annotation_colours <- list(
  'Genotype' = c("Nlrp6WT" = colors_blue,"Nlrp6KO" = colors_orange)
)
...
pdf(file="DESeq2_Heatmap(Genotype+log10)_MAMANTOPOULOS.pdf")
