#This script was used to generate the phylogenetic tree for DSS datasets from Group B.
...
fastafile <- readRDS("fastafile_fully_clustering_DSS_MMUPHin_new.rds")
...
saveRDS(alignment, file = "alignment_fully_clustering_DSS_MMUPHin_new.rds")

phang.align <- phyDat(as(alignment, "matrix"), type="DNA") #Change sequence alignment output into a phyDat structure
saveRDS(phang.align, file = "phang.align_fully_clustering_DSS_MMUPHin_new.rds")

dm <-dist.ml(phang.align) #Create distance matrix
saveRDS(dm, file = "dm_fully_clustering_DSS_MMUPHin_new.rds")

treeNJ <- NJ(dm) #Perform Neighbour joining
saveRDS(treeNJ, file = "treeNJ_fully_clustering_DSS_MMUPHin_new.rds")

fit = pml(treeNJ, data=phang.align) #Internal maximum likelihood
saveRDS(fit, file = "fit_fully_clustering_DSS_MMUPHin_new.rds")

fitGTR <- update(fit, k=4, inv=0.2) #negative edges length changed to 0!
fitGTR <- optim.pml(fitGTR, model="GTR", optInv=TRUE, optGamma=TRUE, rearrangement = "stochastic", control = pml.control(trace=0))

saveRDS(fitGTR, file = "phylogenetic_tree_fully_clustering_DSS_MMUPHin_new.rds")
