##Preprocessing data script for Desai et al., 2016
#######################################################################################################
...
pdf(paste0(fnRs,"qualityprofiles_Rs.pdf"), width=10, height=10)
plotQualityProfile(fnRs[1:2])
dev.off()
pdf(paste0(fnFs,"qualityprofiles_Fs.pdf"), width=10, height=10)
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft=15, truncLen = c(225, 200), maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
save(out, file="filterandtrim_dataset1.Rda") #Saving the output 



#Estimate the error model for DADA2 algorithm using reverse reads
errR <- learnErrors(filtRs, multithread = TRUE)
save(errR, file="errorsR_dataset1.Rda") #Saving the output 
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_dataset1.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()
#Plot error rates for all possible base transitions
pdf("errorplotsR.pdf", width=10, height=10)
...
save(derepFs, file="dereplicationFs_dataset1.Rda") #Saving the output 
derepRs <- derepFastq(filtRs, verbose = TRUE)
save(derepRs, file="dereplicationRs_dataset1.Rda") #Saving the output 
...
save(dadaRs, file="inferenceRs_dataset1.Rda") #Saving the output 
dadaFs <- dada(derepFs, err = errF, multithread = TRUE)
save(dadaFs, file="inferenceFs_dataset1.Rda") #Saving the output


#Merge denoised reads
mergers <- mergePairs(dadaFs, derepFs, dadaRs, derepRs, verbose = TRUE)
head(mergers[[1]])
save(mergers, file="mergeddenoisedreads_dataset1.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab.txt", sep="\t")   
