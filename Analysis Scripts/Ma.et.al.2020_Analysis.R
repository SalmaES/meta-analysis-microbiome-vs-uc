##Analysis Script for Ma et al., 2020
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_F_MA.rds") #feature counts table
taxa <- read_csv("taxtab_F_MA.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_MA.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_F_MA.rds") #phylogenetic tree
...
ps
saveRDS(ps, file = "phyloseq_object_raw_MA.rds")
...
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers_MA.rds")
...
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_MA.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_MA.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_MA.rds")
...
wtp3=pairwise.wilcox.test(rich.pruned$Observed, sample_data(ps.rarefied.pruned)$Intensity_of_Disease)

write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Condition).MA.csv")
write.csv(wtp2[["p.value"]], "Wilcoxon.test.rarefied.pruned(Genotype).MA.csv")
write.csv(wtp3[["p.value"]], "Wilcoxon.test.rarefied.pruned(Intensity_of_Disease).MA.csv")

##Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_MA.pdf")
...
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(Genotype)_MA.pdf")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_MA.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.MA.csv")

beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Genotype", distance = "bray") 
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned(Genotype).MA.csv")
...
write.csv(deseq2_table, "DESeq2_table(Condition)_MA.csv")
...
annotationData <- data.frame(sampledata$Condition, sampledata$Genotype, sampledata$Sample_Accession)
...
colnames(annotationData) <- c('Condition', 'Genotype')
annotation_colours <- list(
  'Condition' = c("Colitogenic" = "#FFC300", "Healthy" = "#1872B0"),
  'Genotype' = c("ASC-/-" =colors_blue, 
                  "GSDMD-/-" =colors_pink, "WT" =colors_green)
)
...
hmap <- Heatmap(
  ...
  column_order = order(annotationData$Condition, annotationData$Genotype),
...
pdf(file="DESeq2_Heatmap_MA(Condition+log10).pdf")
