##Analysis Script for Surana & Kasper, 2017
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_SURANA.rds") #feature counts table
taxa <- read_csv("taxtab_SURANA.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_SURANA.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_SURANA.rds") #phylogenetic tree
...
saveRDS(ps, file = "phyloseq_object_raw_SURANA.rds")
...
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers_SURANA.rds")
...
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_SURANA.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_SURANA.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_SURANA.rds")
...
write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Condition).SURANA.csv")


#Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_SURANA.pdf")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_SURANA.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.SURANA.csv")
...
write.csv(deseq2_table, "DESeq2_table(Condition)_SURANA.csv")
...
#df1 <- subset (df, ef_logFC >= 3 | ef_logFC <= -3)
...
pdf(file="DESeq2_Heatmap_SURANA(Condition).pdf", height=10, width=10)
