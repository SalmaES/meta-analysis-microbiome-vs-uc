##Preprocessing data script for Levy et al., 2015
#######################################################################################################
...
path <- "~/16S_mouse_data/LEVY/PRJEB11300"
...
pdf(paste0(fnRs,"qualityprofiles_Rs_LEVY.pdf"), width=10, height=10)
plotQualityProfile(fnRs[1:2])
dev.off()
pdf(paste0(fnFs,"qualityprofiles_Fs_LEVY.pdf"), width=10, height=10)
...
save(out, file="filterandtrim_LEVY.Rda") #Saving the output 


#Estimate the error model for DADA2 algorithm using reverse reads
errR <- learnErrors(filtRs, multithread = TRUE)
save(errR, file="errorsR_LEVY.Rda") #Saving the output 
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_LEVY.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_LEVY.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()
#Plot error rates for all possible base transitions
pdf("errorplotsR_LEVY.pdf", width=10, height=10)
...
save(derepFs, file="dereplicationFs_LEVY.Rda") #Saving the output 
derepRs <- derepFastq(filtRs, verbose = TRUE)
save(derepRs, file="dereplicationRs_LEVY.Rda") #Saving the output 
...
save(dadaRs, file="inferenceRs_LEVY.Rda") #Saving the output 
dadaFs <- dada(derepFs, err = errF, multithread = TRUE)
save(dadaFs, file="inferenceFs_LEVY.Rda") #Saving the output


#Merge denoised reads
mergers <- mergePairs(dadaFs, derepFs, dadaRs, derepRs, verbose = TRUE)
head(mergers[[1]])
save(mergers, file="mergeddenoisedreads_LEVY.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_LEVY.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_LEVY.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_LEVY.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_LEVY.txt", sep="\t")   
...
write(asv_fasta, "ASVs_LEVY.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_LEVY.tsv", sep="\t", quote=F, col.names=NA)