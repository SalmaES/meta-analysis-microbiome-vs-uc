##Preprocessing data script for Schmidt et al., 2019
#######################################################################################################
...
pdf(paste0(fnFs,"qualityprofiles_Fs_POOL_SCHMIDT.pdf"), width=10, height=10)
plotQualityProfile(fnFs[1:0])
dev.off()
...
save(out, file="filterandtrim_F_SCHMIDT.Rda") #Saving the output 


#Estimate the error model for DADA2 algorithm using reverse reads
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_F_SCHMIDT.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_F_SCHMIDT.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()

#Dereplicate FASTQ files to speed up computation
derepFs <- derepFastq(filtFs, verbose = TRUE)
save(derepFs, file="dereplicationFs_F_SCHMIDT.Rda") #Saving the output 
...
save(dadaFs, file="inferenceFs_F_SCHMIDT.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_F_SCHMIDT.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_F_SCHMIDT.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_F_SCHMIDT.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_F_SCHMIDT.txt", sep="\t")   
...
write(asv_fasta, "ASVs_F_SCHMIDT.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_F_SCHMIDT.tsv", sep="\t", quote=F, col.names=NA)
