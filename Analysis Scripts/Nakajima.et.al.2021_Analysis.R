##Analysis Script for Nakajima et al., 2021
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_F_NAKAJIMA.rds") #feature counts table
taxa <- read_csv("taxtab_F_NAKAJIMA.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_NAKAJIMA.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_F_NAKAJIMA.rds") #phylogenetic tree
...
ps
saveRDS(ps, file = "phyloseq_object_raw_NAKAJIMA.rds")
...
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers_NAKAJIMA.rds")
...
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_NAKAJIMA.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_NAKAJIMA.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_NAKAJIMA.rds")
...
write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Condition).NAKAJIMA.csv")
write.csv(wtp2[["p.value"]], "Wilcoxon.test.rarefied.pruned(Treatment).NAKAJIMA.csv")
write.csv(wtp3[["p.value"]], "Wilcoxon.test.rarefied.pruned(Genotype).NAKAJIMA.csv")


#Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_NAKAJIMA.pdf")
...
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(treatment)_NAKAJIMA.pdf")
plot_ordination(ps.rarefied.pruned, ordination.rarefied.pruned, color = "Treatment", 
                title = "ASVs_Rarefied+Pruned(Bray)") + theme(aspect.ratio=1)
dev.off()

ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray(Genotype)_NAKAJIMA.pdf")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_NAKAJIMA.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.NAKAJIMA.csv")


beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Treatment", distance = "bray") 
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned(treatment).NAKAJIMA.csv")
  
beta.rarified.pruned.bray <- betatest(ps.rarefied.pruned1, group="Genotype", distance = "bray") 
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned(Genotype).NAKAJIMA.csv")


#Differential abundance testing using DESeq2
deseq2 <- microbiomeMarker::run_deseq2(ps.rarefied.pruned1,
                                       group = c("Condition"),
                                       contrast = c("Healthy", "Colitogenic"),
                                       taxa_rank = "none",
                                       sfType = "poscounts",
                                       pvalue_cutoff=0.01, 
                                       transform = "identity", 
...
write.csv(deseq2_table, "DESeq2_table(Condition)_NAKAJIMA.csv")
...
df1 <- subset (df, ef_logFC >= 8 | ef_logFC <= -8)

write_csv(df1, "DESeq2_marker_table_8log10FC_NAKAJIMA(Condition).csv")
...
annotation_colours <- list(
  ...
  'Treatment' = c("Abx+Microbiota_Transfer" =colors_blue, "Abx+PBS" = colors_lilac)
)
colAnnotation <- HeatmapAnnotation(
  ...
  annotation_legend_param =list(
    Treatment = list(
      ...
      at = c("Abx+Microbiota_Transfer", "Abx+PBS"),
      labels = c("Abx+Microbiota Transfer", "Abx+PBS")
    )))
...
pdf(file="DESeq2_Heatmap_NAKAJIMA(Condition+log10).pdf", height=10)
