...
path <- "~/16S_mouse_data/GERASSY-VAINBERG"
...
pdf(paste0(fnFs,"qualityprofiles_Fs_GERASSY-VAINBERG.pdf"), width=10, height=10)
...
out <- filterAndTrim(fnFs, filtFs, trimLeft=15, truncLen = 200, maxN=0, maxEE=3, truncQ=3, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
save(out, file="filterandtrim_GERASSY-VAINBERG.Rda") #Saving the output 

exists <- file.exists(filtFs)
filtFs <- filtFs[exists]
sample.names1 <- sapply(strsplit(filtFs, "_"), `[`,1)

#Estimate the error model for DADA2 algorithm using reverse reads
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_GERASSY-VAINBERG.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_GERASSY-VAINBERG.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()

#Dereplicate FASTQ files to speed up computation
derepFs <- derepFastq(filtFs, verbose = TRUE)
save(derepFs, file="dereplicationFs_GERASSY-VAINBERG.Rda") #Saving the output 
...
save(dadaFs, file="inferenceFs_GERASSY-VAINBERG.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_GERASSY-VAINBERG.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_GERASSY-VAINBERG.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_GERASSY-VAINBERG.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_GERASSY-VAINBERG.txt", sep="\t")   
...
write(asv_fasta, "ASVs_GERASSY-VAINBERG.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_GERASSY-VAINBERG.tsv", sep="\t", quote=F, col.names=NA)
