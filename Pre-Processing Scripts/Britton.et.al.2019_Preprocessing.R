##Preprocessing data script for Britton et al., 2019
#######################################################################################################
...
pdf(paste0(fnFs,"qualityprofiles_Fs_BRITTON.pdf"), width=10, height=10)
...
save(out, file="filterandtrim_BRITTON.Rda") #Saving the output 


#Estimate the error model for DADA2 algorithm using reverse reads
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_BRITTON.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_BRITTON.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()

#Dereplicate FASTQ files to speed up computation
derepFs <- derepFastq(filtFs, verbose = TRUE)
save(derepFs, file="dereplicationFs_BRITTON.Rda") #Saving the output 
...
save(dadaFs, file="inferenceFs_BRITTON.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_BRITTON.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_BRITTON.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_BRITTON.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_BRITTON.txt", sep="\t")   
...
write(asv_fasta, "ASVs_BRITTON.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_BRITTON.tsv", sep="\t", quote=F, col.names=NA)
