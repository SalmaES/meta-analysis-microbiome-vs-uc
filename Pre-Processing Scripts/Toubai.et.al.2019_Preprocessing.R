##Preprocessing data script for Toubai et al., 2019
#######################################################################################################
...
pdf(paste0(fnRs,"qualityprofiles_Rs_TOUBAI.pdf"), width=10, height=10)
plotQualityProfile(fnRs[1:2])
dev.off()
pdf(paste0(fnFs,"qualityprofiles_Fs_TOUBAI.pdf"), width=10, height=10)
...
save(out, file="filterandtrim_TOUBAI.Rda") #Saving the output 
...
save(errR, file="errorsR_TOUBAI.Rda") #Saving the output 
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_TOUBAI.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_TOUBAI.pdf", width=10, height=10)
plotErrors(errF, nominalQ=TRUE)
dev.off()
#Plot error rates for all possible base transitions
pdf("errorplotsR_TOUBAI.pdf", width=10, height=10)
...
save(derepFs, file="dereplicationFs_TOUBAI.Rda") #Saving the output 
derepRs <- derepFastq(filtRs, verbose = TRUE)
save(derepRs, file="dereplicationRs_TOUBAI.Rda") #Saving the output 
...
save(dadaRs, file="inferenceRs_TOUBAI.Rda") #Saving the output 
dadaFs <- dada(derepFs, err = errF, multithread = TRUE)
save(dadaFs, file="inferenceFs_TOUBAI.Rda") #Saving the output


#Merge denoised reads
mergers <- mergePairs(dadaFs, derepFs, dadaRs, derepRs, verbose = TRUE)
head(mergers[[1]])
save(mergers, file="mergeddenoisedreads_TOUBAI.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_TOUBAI.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_TOUBAI.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_TOUBAI.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_TOUBAI.txt", sep="\t")   
...
write(asv_fasta, "ASVs_TOUBAI.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_TOUBAI.tsv", sep="\t", quote=F, col.names=NA)