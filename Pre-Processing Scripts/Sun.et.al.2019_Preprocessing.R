##Preprocessing data script for Sun et al., 2019
#######################################################################################################
...
pdf(paste0(fnFs,"qualityprofiles_Fs_175_SUN.pdf"), width=10, height=10)
...
out <- filterAndTrim(fnFs, filtFs, trimLeft=10,  truncLen=180, maxN=0, maxEE=2, truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
save(out, file="filterandtrim_F_SUN.Rda") #Saving the output 



#Estimate the error model for DADA2 algorithm using reverse reads 
errF <- learnErrors(filtFs, multithread = TRUE)
save(errF, file="errorsF_F_SUN.Rda") #Saving the output 


#Plot error rates for all possible base transitions
pdf("errorplotsFs_F_SUN.pdf", width=10, height=10)
...
save(derepFs, file="dereplicationFs_F_SUN.Rda") #Saving the output 
...
save(dadaFs, file="inferenceFs_F_SUN.Rda") #Saving the output
...
write.table(seqtab, file = "seqtab_F_SUN.txt", sep="\t")
...
write.table(seqtab.nochim, file = "seqtabnoc_F_SUN.txt", sep="\t")
...
write.table(summary_table, "read-count-tracking_F_SUN.tsv", quote = FALSE, sep = "\t", col.names = NA)
...
write.table(taxa.plus, file = "taxtab_F_SUN.txt", sep="\t")   
...
write(asv_fasta, "ASVs_F_SUN.fa")

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_F_SUN.tsv", sep="\t", quote=F, col.names=NA)
