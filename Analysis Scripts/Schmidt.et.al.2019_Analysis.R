##Analysis Script for Schmidt et al., 2019
#######################################################################################################
#Load R libraries
library(dada2)
library(DECIPHER)
...
OTU <- readRDS("seqtabnoc_F_SCHMIDT.rds") #feature counts table
taxa <- read_csv("taxtab_F_SCHMIDT.csv") #taxonomy table
sampledata <- read_csv("adjusted_sampledata_SCHMIDT.csv") #metadata
tree <- readRDS("Phylogenetic_Tree_F_SCHMIDT.rds") #phylogenetic tree
...
ps
saveRDS(ps, file = "phyloseq_object_raw_SCHMIDT.rds")
...
taxa_names(ps2)[1:2]
ps2
saveRDS(ps2, file = "phyloseq_raw_no_outliers_SCHMIDT.rds")

#Rarefy the data
set.seed(19920921)
ps.rarefied <- rarefy_even_depth(ps2, rngseed=FALSE, replace=T)
ps.rarefied
saveRDS(ps.rarefied, file = "phyloseq_rarefied_SCHMIDT.rds")
...
pdf(file="Prevalence_Filtering_Plots(Rarefied)_SCHMIDT.pdf")
...
ps.rarefied.pruned
saveRDS(ps.rarefied.pruned, file = "phyloseq_object_rarefied_pruned_SCHMIDT.rds")
...
write.csv(wtp1[["p.value"]], "Wilcoxon.test.rarefied.pruned(Condition).SCHMIDT.csv")

#Ordination of rarefied and pruned data
ordination.rarefied.pruned <- ordinate(ps.rarefied.pruned, "PCoA", distance = "bray")
pdf(file="PCoA_Rarified_Pruned_Bray_SCHMIDT.pdf")
...
ps.rarefied.pruned1 <- ps.rarefied.pruned %>%
  ...
  rename_with_tax_table(stringr::str_to_title)
saveRDS(ps.rarefied.pruned1, file = "phyloseq_rarefied_pruned1_SCHMIDT.rds")
...
write.csv(beta.rarified.pruned.bray, "PERMANOVA.test.bray.rarefied.pruned.SCHMIDT.csv")


#Differential abundance testing with DESeq2
ps.rarefied.pruned1 <- readRDS("phyloseq_rarefied_pruned1_SCHMIDT.rds")
...
write.csv(deseq2_table, "DESeq2_table(Condition)_SCHMIDT.csv")
...
annotation_colours <- list(
  'Condition' = c("Healthy" = "#1872B0","Colitis" = "#FA7F4A")
)
...
pdf(file="DESeq2_Heatmap(Condition+log10)_SCHMIDT.pdf")
